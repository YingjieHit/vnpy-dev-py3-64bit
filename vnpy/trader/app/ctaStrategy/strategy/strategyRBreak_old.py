...
class RBreakStrategy(CtaTemplate):
    ...
    def onFifteenBar(self, bar):
        ...
        SIZECK1 = abs(bar.close - bar.open) / bar.close < 5 / 1000 # SIZECK1 = (当前BAR收盘价 - 当前BAR开盘价)的绝对值/当前BAR收盘价   < 0.5%  振幅不超过0.5%
        ...
        if self.pos == 0:
            ...
            if time_now >= time_0915 and time_now <= time_1429:
                ...
                if am.close[-1] < U6 and am.close[-2] >= U6 and am.close[-3] > U6 and am.close[-31: -1].max() > U5 and SIZECK2:
                    self.cancelAll()
                ...
                if am.close[-1] > D6 and am.close[-2] <= D6 and am.close[-3] < D6 and am.close[-31: -1].min() < D5 and SIZECK1:
                    self.cancelAll()
        ...
        # 持有多头：
            ...
            elif self.intraTradeHigh > self.open_price * (1 + 0.002) and (am.close[-1] - self.open_price) <= (self.intraTradeHigh - self.open_price) * 0.5:
                self.cancelAll()
                self.sell(price=bar.close - 0.01, volume=self.fixedSize)
            # 价格下穿 开仓价格 * (1 - 0.9/1000) 止损
            elif am.close[-1] < self.open_price * (1 - 0.9 / 1000)  and am.close[-2] >= self.open_price * (1 - 0.9 / 1000):
                self.cancelAll()
                self.sell(price=bar.close - 0.01, volume=self.fixedSize)
        ...
        # 持有空头;
            ...
            elif self.intraTradeLow < self.open_price * (1 - 0.002) and self.open_price - bar.close <= (self.open_price - self.intraTradeLow) * 0.5:
                self.cancelAll()
                self.cover(price=bar.close + 0.01, volume=self.fixedSize)
