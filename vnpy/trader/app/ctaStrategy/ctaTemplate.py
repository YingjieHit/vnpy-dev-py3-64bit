...
class BarManager(object):
    ...
    def updateBar(self, bar):
        ...
        if not (bar.datetime.minute + 1) % self.xmin:  # 可以用X整除，原写法会造成早上开盘第一根bar就算合成了一根bar，张英杰修改
            # 生成上一X分钟K线的时间戳
...
class ArrayManager2(ArrayManager):

    # ----------------------------------------------------------------------
    def __init__(self, size=100):
        """Constructor"""
        super(ArrayManager2, self).__init__(size)
        # self.count = 0  # 缓存计数
        # self.size = size  # 缓存大小
        # self.inited = False  # True if count>=size
        #
        ...
        # self.closeArray = np.zeros(size)
        # self.volumeArray = np.zeros(size)
        self.dateArray = np.array(["19900101" for i in range(0, size)])
        self.timeArray = np.array(["00:00:00" for i in range(0, size)])
        self.datetimeArray = np.array([datetime.datetime.strptime("19900101", "%Y%m%d") for i in range(0, size)])

    # ----------------------------------------------------------------------
    def updateBar(self, bar):
        """更新K线"""
        super(ArrayManager2, self).updateBar(bar)

        self.dateArray[0:self.size - 1] = self.dateArray[1:self.size]
        self.timeArray[0:self.size - 1] = self.timeArray[1:self.size]
        self.datetimeArray[0:self.size - 1] = self.datetimeArray[1:self.size]

        self.dateArray[-1] = bar.date
        self.timeArray[-1] = bar.time
        self.datetimeArray[-1] = bar.datetime
...
class DailyArrayManager(ArrayManager):
    """
    加入日k线序列管理工具，加入功能：
    1. 日K线时间序列管理
    2. 日K线常用技术指标计算
    ...
    def __init__(self, size=4):
        """Constructor"""
        super(DailyArrayManager, self).__init__(size)
        # self.count = 0  # 缓存计数
        # self.size = size  # 缓存大小
        # self.inited = False  # True if count>=size
        #
        ...
        # self.closeArray = np.zeros(size)
        # self.volumeArray = np.zeros(size)
        self.openInterestArray = np.zeros(size)
    ...
    def updateBar(self, bar):
        ...
        if self.today is None:
            ...
            self.closeArray[-1] = bar.close
            self.volumeArray[-1] = bar.volume
            self.openInterestArray[-1] = bar.openInterest

        # 新的一天到了
        elif self.today != bar.date:
            self.pre_day = self.today
            self.today = bar.date
            self.count += 1
            ...
            self.closeArray[0:self.size - 1] = self.closeArray[1:self.size]
            self.volumeArray[0:self.size - 1] = self.volumeArray[1:self.size]
            self.openInterestArray[0:self.size - 1] = self.openInterestArray[1:self.size]
    ...
    @property
    def pre_date(self):
        return self.pre_date


class BarManager2(BarManager):
    """bar管理器升级版本，此版本所有尾盘都会提前n（5）秒钟判断和发信号"""

    def __init__(self, onBar, xmin=0, onXminBar=None, **kwargs):

        super(BarManager2, self).__init__(onBar, xmin, onXminBar)
        # 停止交易时间，默认为国债的
        self.close_time_list = kwargs.get("close_time_list", [datetime.time(hour=11, minute=29, second=54), datetime.time(hour=15, minute=14, second=54)])
        # 开盘时间，用于重置进入收盘准备标记位
        self.open_time_list = kwargs.get("open_time_list", [datetime.time(hour=9, minute=15, second=0), datetime.time(hour=13, minute=0, second=0)])
        self.ready_close = False  # 进入收盘准备标记位
    ...
    def updateTick(self, tick):
        ...
        newMinute = False  # 默认不是新的一分钟
        # 开盘后重置标记位
        if tick.datetime.time() in self.open_time_list:
        ...
        # 快收盘了
        elif tick.datetime.time() in self.close_time_list and self.ready_close is False:
            # 生成上一分钟K线的时间戳
            self.bar.datetime = self.bar.datetime.replace(second=0, microsecond=0)  # 将秒和微秒设为0
            self.bar.date = self.bar.datetime.strftime('%Y%m%d')
            self.bar.time = self.bar.datetime.strftime('%H:%M:%S.%f')
